module.exports = class GameEngine {
  init(game, imageManager, soundManager, fps) {
    this.game = game;
  ...
  addListeners() {
    this.theCanvas.addEventListener(
      'mousedown',
    ...
    this.theCanvas.addEventListener(
      'touchstart',
      this.touchDownListener.bind(this),
      false
    );
    ...
    window.addEventListener(
      'keydown',
      event => {
        this.game.keyDownListener(event.key);
      },
      true
    );
    window.addEventListener(
      'keypress',
      event => {
        this.game.keyPressListener(event.key);
      },
      true
    );
    window.addEventListener(
      'keyup',
      event => {
        this.game.keyUpListener(event.key);
      },
      true
    );
  }
  ...
  mouseDownListener(evt) {
    const bRect = this.theCanvas.getBoundingClientRect();
    const touchX =
      (evt.clientX - bRect.left) * (this.theCanvas.width / bRect.width);
    const touchY =
      (evt.clientY - bRect.top) * (this.theCanvas.height / bRect.height);
    this.game.inputDownListener(touchX, touchY);
  }

  touchDownListener(evt) {
    evt.preventDefault();
    evt.stopPropagation();
    const bRect = this.theCanvas.getBoundingClientRect();
    const touches = evt.changedTouches;
    const touchX =
      (touches[0].pageX - bRect.left) * (this.theCanvas.width / bRect.width);
    const touchY =
      (touches[0].pageY - bRect.top) * (this.theCanvas.height / bRect.height);
    this.game.inputDownListener(touchX, touchY);
  }
